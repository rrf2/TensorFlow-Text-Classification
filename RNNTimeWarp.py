...
initialState = tf.placeholder(tf.float32, [batchSize, hiddenUnits], name="initialState")
...
pastStateList = []
pastStateList.append(currentState)

specialVector = tf.Variable(np.zeros((batchSize, hiddenUnits)), dtype=tf.float32)

i = 0
for lineMatrix in sequenceOfLetters:
    #
    # concatenate the state with the input, then compute the next state
    if i < 10:
        inputPlusState = tf.concat([lineMatrix, currentState, specialVector], 1)
    else:
        inputPlusState = tf.concat([lineMatrix, currentState, pastStateList[i-10]], 1)
...
with tf.Session() as sess:
    ...
    for epoch in range(numTrainingIters):
        ...
        if (numTrainingIters - epoch) <= 20:
            ...
            if (numTrainingIters - epoch) <= 10:
                last10.append((_totalLoss, numCorrect))
...
for i in range(10):
    sumLoss += last10[i][0]
    sumNumCorrect += last10[i][1]

print("Average loss for the last 10 mini-batches is " + str(sumLoss/10.0) + ", average correct labels is " + str(sumNumCorrect/10.0) + " out of " + str(batchSize))
